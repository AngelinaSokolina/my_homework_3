...
def test_mask_account(account_number, expected_account):
    ...
    # ______________БЛОК ТЕСТА С ФИКСТУРОЙ______________
def test_mask_account_executed(standard_account_number):
    result = get_mask_account(standard_account_number)
    assert result == "**5640"


def test_incorrect_account_executed(incorrect_input):
    assert get_mask_account(incorrect_input) == "Ошибка: номер счета должен состоять из 20 цифр"

def test_empty_account_executed(empty_input):
